...
namespace nStack.Helpers
{
    public class FileReader: BinFileObject
    {
        ...
        {
            //Dictionary<string, Dictionary<string, List<string>>>[] container = new Dictionary<string, Dictionary<string, List<string>>>[3];
            ...
            if (BinFile.Exists)
            {
                using (var binFile = BinFile.OpenRead())
                {
                    ...
                    while (binPosition < binEnd)
                    {
                       ...
                       // var tt = binaryFormatter.Deserialize(binFile);
